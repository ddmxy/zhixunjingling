...
SAVE_DIR = "./calib_images3"          # 图片保存到当前目录下的 calib_images 文件夹
IMAGE_TOPIC = "/image_raw"           # 由 hbm_to_raw.py 发布的标准话题
...
class CalibCmdCollector(Node):
    def __init__(self):
        ...
        self.get_logger().info("标定图像采集已启动")
    ...
    def keyboard_listener(self):
        while self.running:
            ...
            if cmd.lower() == 'q':
                self.running = False
                self.get_logger().info("退出采集")
                rclpy.shutdown()
                return
            # 回车（空字符串）触发保存
            if self.latest_frame is not None:
                save_path = f"{SAVE_DIR}/calib3_{self.count:02d}.jpg"
