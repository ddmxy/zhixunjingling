		...
		while (rclcpp::ok())
		{
			...
			if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE")
			{
				if (pubScan)
				{
					...
					{
						// Anchor stamp to publish-time clock, offset back by half sweep.
						// Avoids TF extrapolation errors (past/future) seen with this->now()
						// or stale pre_time_ from the serial receive thread.
						const double sweep_sec = std::max(0.05, static_cast<double>(scan_time));
						scan->header.stamp =
							this->now() - rclcpp::Duration::from_seconds(sweep_sec * 0.5);
					}
					scan->angle_min = 0;
